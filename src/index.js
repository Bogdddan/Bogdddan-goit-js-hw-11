...
btn.addEventListener('click', () => {
  ...
  fetchCountries(searchQuery, page)
    .then(data => {
      renderImages(data.hits);
      if (data.totalHits <= divForRender.children.length) { // додано перевірку на кінець колекції
        btn.style.display = 'none';
        Notiflix.Notify.info("We're sorry, but you've reached the end of search results.");
      } else {
        btn.style.display = 'block';
...
function renderImages(images) {
  ...
  <div class="image-card"> 
    <img src="${webformatURL}" alt="${tags}" class="image-card__image" data-source="${largeImageURL}"> 
      <div class="image-card__stats"> 
          <p class="image-card__stats-item">
